...
SceneCreationException::SceneCreationException(std::string const& s) : std::runtime_error(s)
{}

Tile Tile::from(json11::Json const& json)
{
    Tile tile {};
...
json11::Json Tile::to_json() const
{
...
DrawableBuffer::DrawableBuffer(sf::Texture const& texture) : texture(texture)
{}

void DrawableBuffer::add(Box<int> const& position_in_sheet, Box<int> const& physical_position)
{
    add_to_vertex_array_quads(position_in_sheet, physical_position, vertex_array);
}
void DrawableBuffer::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
...
Scene::Scene(std::string const& full_path, jeagle::PhysicsWorld* physics) : scene_path(full_path), physics(physics)
{
...
Scene Scene::create_empty(std::string const& file_path)
{
    Scene scene{};
...
void Scene::add_tile(Box<int> const& sprite_box, std::optional<Box<int>> const& collision_box, std::string const& tileset_path, Box<int> const& position_in_texture)
{
...
void Scene::export_to_file(std::string const& path)
{
...
//private
void Scene::add_to_drawables(Tile const& tile, sf::Texture const& texture)
{
...
void Scene::add_collision_box(Tile const& tile)
{
